...
export function ReportGate({ profile, children }: ReportGateProps) {
    ...
    return (
      <div className="border border-ink border-dashed p-12 text-center space-y-6">
        <p className="font-headline text-2xl font-bold text-ink">
          {messages.reporting.signInRequired}
        </p>
        <Link
          href={`/${locale}/account`}
  ...
  if (!demoMode && !canAccess(profile!.planName, "canReport")) {
    return (
      <div className="border border-ink border-dashed p-12 text-center space-y-6">
        <p className="font-headline text-2xl font-bold text-ink">
          {messages.reporting.requiresPlan}
        </p>
        <Link
          href={`/${locale}/account`}
          className="inline-block bg-primary text-white px-8 py-3 font-label text-[0.6875rem] font-bold uppercase tracking-[0.2em] hover:bg-ink transition-colors"
  ...
  if (!demoMode && !profile!.isVerifiedVeterinarian) {
    return (
      <div className="border border-ink border-dashed p-12 text-center space-y-6">
        <p className="font-headline text-2xl font-bold text-ink">
          {messages.reporting.requiresVerifiedVeterinarian}
        </p>
        <Link
          href={`/${locale}/account`}
          className="inline-block bg-primary text-white px-8 py-3 font-label text-[0.6875rem] font-bold uppercase tracking-[0.2em] hover:bg-ink transition-colors"
        >
          {messages.account.reviewAccess}
        </Link>
      </div>
    );
  }
